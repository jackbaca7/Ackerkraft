...
func _ready():
	if Global.new_game:
		...
		for x in range(tile_map_size["min_x"], tile_map_size["max_x"]):
			for y in range(tile_map_size["min_y"], tile_map_size["max_y"]):
				...
				set_cell(int(Global.GROUND), vector, Global.GROSS, Vector2i(0,0))
				SaveSystem.add_tile_data(Global.GROUND, vector, {"tile": Global.GROSS, "position": {"x" : x, "y" : y}})
				
		set_cell(int(Global.BULDING), Vector2i(3, 3), int(Global.BLACKSMITH), Vector2i(0,0))
		...
		set_cell(int(Global.BULDING), Vector2i(-2, -3), int(Global.BLACKSMITH), Vector2i(0,0))
		...
		set_cell(int(Global.BULDING), Vector2i(2, 1), int(Global.BLACKSMITH), Vector2i(0,0))
		...
		set_cell(int(Global.BULDING), Vector2i(1, 2), int(Global.BLACKSMITH), Vector2i(0,0))
		...
		set_cell(int(Global.BULDING), Vector2i(3, -4), int(Global.WATER_WELL), Vector2i(0,0))
		...
		if list_tile_map != null:
			...
			for layer in list_tile_map.keys():
				...
				for tile_position in list_tile_map[layer].keys():
					...
					set_cell(int(layer), position_tile_vec, int(tile_data["tile"]), Vector2i(0,0))
	...
	var tilemap_size = Vector2i(40,40)
	map_rect = Rect2i(Vector2i(-20,-20), tilemap_size)
	
	var tile_size = get_tileset().tile_size
	
	var tile_map_size : Dictionary = {
		"max_x": 10,
		"min_x": -10,
		"max_y": 10,
		"min_y": -10
...
func _process(delta):
	var mouse_tile = local_to_map(get_global_mouse_position())
	if Global.preview_active and last_preview_pos != mouse_tile and Global.current_bulding_tile != "-1":
		set_cell(int(Global.PREVIEW), last_preview_pos, -1)
		set_cell(int(Global.PREVIEW), mouse_tile, int(Global.current_bulding_tile), Vector2i(0,0))
...
func check_tile_place(place_position):
	var data
	if Global.current_bulding_tile == "-1":
		return false
	
	for layer in Global.LAYERS:
		data = SaveSystem.get_tile_data(place_position, layer)
		if ((layer == Global.GROUND and data == null) or (layer != Global.GROUND and data)):
			return false
	print("cleartoplace: ", true)
	return true


func place_bulding():
	if check_tile_place(last_preview_pos):
		set_cell(int(Global.BULDING), last_preview_pos, int(Global.current_bulding_tile), Vector2i(0,0))
		...
		SaveSystem.add_tile_data(Global.BULDING, last_preview_pos, 
			{"tile": Global.current_bulding_tile, "duration": Global.task_speed_tile[Global.current_bulding_tile], "position": {"x" : last_preview_pos.x, "y" : last_preview_pos.y}})
		SaveSystem.save()

func clear_preview():
	clear_layer(int(Global.PREVIEW))
