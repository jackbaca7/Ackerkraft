...
@export var ignoreX = true
@export var ignoreY = true

@export_category("Move when Target outside Camera")
@export var boundsMovement = false # TODO make it smoff
@export var boundsPosition = Vector2(2000, 2000)
...
# Added mouse sensitivity for movement.
@export var mouse_sensitivity = 3
# Added zoom sensitivity, controls how much the zoom changes on wheel scroll.
@export var zoom_sensitivity = 0.1

@onready var worker_stats_panel = get_node("/root/Game/UI/Control/worker_stats_view/worker_stats_panel")


func _ready():
	# Stellen Sie sicher, dass die Kamera nicht außerhalb der Grenzen Ihres Spiels verschoben werden kann.
	# Diese Einstellungen hängen von der Größe Ihrer Spielwelt ab.
	limit_left = -20000
	limit_top = -20000
	limit_right = 20000
	limit_bottom = 20000
...
func _input(event):
	...
	if event is InputEventMouseButton:
		usingMouse = true
	# Prüfe, ob die linke Maustaste gedrückt wurde.
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_MIDDLE:
			if event.pressed:
				# Starte den Drag-Vorgang
				drag_origin = event.position * mouse_sensitivity + global_position
				dragging = true
			else:
				# Beende den Drag-Vorgang
				dragging = false

				# Prüfe, ob das Mausrad benutzt wird, um zu zoomen.
	if event is InputEventMouseMotion:
		if dragging:
			# Aktualisiere die Position der Kamera basierend auf der Bewegung der Maus.
			global_position = drag_origin - event.position * mouse_sensitivity

	if event is InputEventMouseButton and !worker_stats_panel.visible:
